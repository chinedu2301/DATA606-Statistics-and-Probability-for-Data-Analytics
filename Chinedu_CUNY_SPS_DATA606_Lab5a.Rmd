---
title: "CUNY SPS DATA606 Lab5a: Foundations for statistical inference - Sampling distributions"
...
Check the summary statistics of samp1
```{r}
samp1 %>%
  count(scientist_work) %>%
...
sample_props50 <- global_monitor %>% 
  rep_sample_n(size = 50, reps = 15000, replace=TRUE) %>% count(scientist_work) %>%
  mutate(p_hat = n/sum(n)) %>% 
  filter(scientist_work == "Doesn't benefit")
...
Using this sample, the best point estimate of the population proportion of people who think the work scientists do enhances their lives is 0.867
